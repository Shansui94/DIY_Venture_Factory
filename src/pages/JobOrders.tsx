...
import { JobOrder, PRODUCT_LAYERS, PRODUCT_MATERIALS, PACKAGING_COLORS, PRODUCT_SIZES } from '../types';
import { getRecipesByProduct } from '../services/productionService';
...
const JobOrders: React.FC<JobOrdersProps> = ({ jobs, onCreateJob, onReorderJobs }) => {
    ...
    React.useEffect(() => {
        ...
        if (layer && material && size) {
            // Import dynamically or ensure import exists at top
            import('../utils/packagingRules').then(({ getRecommendedPackaging }) => {
        ...
        if (layer && size && material && packaging) {
            const fetchRecipes = async () => {
                ...
                try {
                    ...
                    const { getItemBySku } = await import('../services/productionService');
