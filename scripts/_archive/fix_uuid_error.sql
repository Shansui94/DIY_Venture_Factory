...
-- 1. Modify Table to support TEXT machine IDs (e.g. "T1.2-M01")
ALTER TABLE production_logs_v2 
    ALTER COLUMN machine_id TYPE TEXT USING machine_id::text;

-- 2. Modify RPC to explicitly accept TEXT and insert TEXT
CREATE OR REPLACE FUNCTION execute_production_run_v3(
    p_sku TEXT,
    p_qty DECIMAL,
    p_operator_id UUID DEFAULT NULL,
    p_machine_id TEXT DEFAULT NULL, -- Explicitly TEXT
    p_scale_factor DECIMAL DEFAULT 1.0,
    p_job_id TEXT DEFAULT NULL,    -- Add missing args if any
...
BEGIN
    ...
    FROM bom_headers_v2 WHERE sku = p_sku AND is_default = true LIMIT 1;

    IF v_recipe_id IS NULL THEN
        SELECT recipe_id INTO v_recipe_id FROM bom_headers_v2 WHERE sku = p_sku LIMIT 1;
    END IF;
