...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    try {
        ...
        if (!mac && req.method === 'GET') {
            // Original Logic: Fetch all products if no MAC provided
        ...
        // --- IoT Logic ---
        const supabaseUrl = process.env.VITE_SUPABASE_URL || "https://kdahubyhwndgyloaljak.supabase.co";
        const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtkYWh1Ynlod25kZ3lsb2FsamakIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjUzODY4ODksLCJleHAiOjIwNjUzODY4ODl9.mzTtQ6zpfvRY07372UH_M4dvKPzHBDkiydwosUYPs-8";
        const supabase = createClient(supabaseUrl, supabaseKey);
